...
const Dashboard: React.FC<DashboardProps> = ({ user, onLogout, onLoginReq }) => {
  ...
  const handleTutorial = (title: string, desc: string) => {
    if (!user) { setTutorialMessage({ title, desc }); return true; }
    return false;
  };
  ...
  const handleDeleteHabit = async (record: HabitRecord) => {
    ...
    if (!user || !confirm(`'${record.habit.name}' 습관을 삭제하시겠습니까?`)) return;
  ...
  const handleToggleLog = async (record: HabitRecord) => {
    if (!user) {
       handleTutorial("습관 기록", "오늘의 목표를 달성했나요? 버튼을 눌러 O, X, 또는 취소 상태로 기록할 수 있어요.");
  ...
  const getWeeklyRate = (record: HabitRecord) => {
    ...
    let dueCount = 0; let successCount = 0;
    for(let i=0; i<7; i++) {
       const d = new Date(startOfWeek);
       d.setDate(startOfWeek.getDate() + i);
       const ds = getLocalISOString(d);
       let isDue = record.habit.frequency.type === 'daily' || (record.habit.frequency.type === 'specific_days' && record.habit.frequency.days?.includes(d.getDay()));
       if (record.habit.frequency.type === 'weekly_count' && i === 6) dueCount = record.habit.frequency.value || 1;
       if (record.habit.frequency.type !== 'weekly_count' && isDue) dueCount++;
       if (record.logs[ds] === true) successCount++;
    }
    return dueCount === 0 ? 0 : Math.round(Math.min(100, (successCount / dueCount) * 100));
  };
  ...
  return (
    <div className="h-screen bg-github-bg text-github-text font-sans flex flex-col overflow-hidden">
                 ...
                 <div className="space-y-3">
                   {invitedHabits.map(({ myRecord: record }) => (
                       <div key={record.habit_id} className="flex items-center justify-between bg-github-bg p-3 rounded border border-github-border">
                          ...
                          <div className="flex gap-2"><button onClick={() => handleInviteResponse(record, true)} className="px-3 py-1.5 bg-github-success text-white rounded text-xs font-bold hover:bg-github-successHover">수락</button><button onClick={() => handleInviteResponse(record, false)} className="px-3 py-1.5 bg-github-btn text-github-muted rounded text-xs hover:text-red-400">거절</button></div>
                       </div>
                   ))}
                 </div>
              </section>
              ...
              {loading ? <div className="h-48 flex items-center justify-center text-github-muted"><Loader2 className="animate-spin mr-2" /> 기록을 불러오는 중...</div> : (
                <div className="space-y-3">
                  ...
                  {todaysHabits.map(({ myRecord: record }) => {
                    ...
                    const isDone = status === true; const isFail = status === false;
                    return (
                      <div key={record.habit_id} className={`bg-github-card border ${isDone ? 'border-github-muted' : (isFail ? 'border-red-900/30' : 'border-github-border')} rounded-lg p-4 flex items-center justify-between transition-all`}>
                        <div className="flex items-center gap-4"><div className={`w-12 h-12 rounded-xl flex items-center justify-center text-white font-bold shadow-sm ${isDone ? record.habit.color : (isFail ? 'bg-red-900/50 text-red-200' : `${record.habit.color} opacity-40`)}`}>{isFail ? <XIcon size={24} /> : record.habit.name.charAt(0)}</div><div><h3 className={`font-semibold text-lg ${isDone ? 'text-github-muted line-through' : 'text-github-text'}`}>{record.habit.name}</h3><p className="text-xs text-github-muted">{record.habit.type === 'do' ? '목표: ' : '제한: '} {record.habit.goal} {record.habit.unit}</p></div></div>
                        <button onClick={() => handleToggleLog(record)} className={`w-12 h-12 rounded-xl border flex items-center justify-center transition-all ${isDone ? 'bg-github-btn border-github-muted text-github-muted' : (isFail ? 'bg-red-900/10 border-red-900/30 text-red-400' : 'bg-github-btn border-github-border text-github-success hover:bg-github-btnHover')}`}>{isDone ? <XIcon size={24} /> : (isFail ? <Minus size={24} /> : <Plus size={24} />)}</button>
                      </div>
                    );
                  })}
                  <button onClick={handleAddClick} className="w-full py-4 border-2 border-dashed border-github-border rounded-lg text-github-muted hover:text-github-text hover:border-github-muted hover:bg-github-card/50 transition-all flex items-center justify-center gap-2 font-medium mt-4"><Plus size={18} />새로운 습관 추가하기</button>
                </div>
              )}
            </section>
          </div>
      ...
      {showHabitForm && user && <HabitForm userEmail={user.email} onClose={() => setShowHabitForm(false)} onSave={handleSaveHabit} initialData={editingHabit} />}
      {tutorialMessage && (

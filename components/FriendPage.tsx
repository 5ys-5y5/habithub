...
const FriendPage: React.FC<FriendPageProps> = ({ user, onLogout, onNavigate, onViewFriend }) => {
  ...
  return (
    <div className="h-screen bg-github-bg text-github-text font-sans flex flex-col overflow-hidden relative">
      {toast && (
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar w-full">
        <main className="max-w-md w-full mx-auto p-4 grid grid-cols-1 gap-6">
           ...
           <div className="space-y-6">
              <section className="bg-github-card border border-github-border rounded-xl p-6 shadow-sm">
                 <div className="flex flex-row items-center gap-4">
                    <div className="w-20 h-20 bg-gradient-to-br from-github-accent to-blue-700 rounded-full flex items-center justify-center text-3xl font-bold text-white shadow-lg flex-shrink-0">
                       {user.name.charAt(0)}
                    </div>
                    <div className="flex flex-col items-start min-w-0">
                       <h2 className="text-xl font-bold text-white truncate max-w-[180px]">{user.name}</h2>
                       <p className="text-sm text-github-muted truncate max-w-[180px]">{user.email}</p>
                    </div>
                 </div>
              </section>
              ...
              <section className="bg-github-card border border-github-border rounded-xl p-6 shadow-sm relative overflow-hidden">
                 ...
                 <div className="space-y-3 relative z-10">
                    {loading && rankings.length === 0 ? (
                       <div className="text-center py-8 text-github-muted flex flex-col items-center">
                          <Loader2 className="animate-spin mb-2" />
                          <span className="text-xs">랭킹 불러오는 중...</span>
                       </div>
                    ) : rankings.length === 0 ? (
                       <p className="text-sm text-github-muted text-center py-4">순위 데이터가 없습니다.</p>
                    ) : (
                       rankings.map((r) => (
                       ...
                       <div className="space-y-4">
                          ...
                          {loading && acceptedFriends.length === 0 ? (
                             <div className="flex flex-col items-center justify-center py-20 text-github-muted">
                                ...
                                <p className="text-sm">친구 목록을 불러오는 중...</p>
                             </div>
                    ...
                    {activeTab === 'requests' && (
                       <div className="space-y-6">
                          <div>
                             ...
                             {loading ? (
                                <div className="py-8 text-center text-github-muted flex items-center justify-center gap-2"><Loader2 className="animate-spin" size={16}/>불러오는 중...</div>

...
const FriendSidebar: React.FC<FriendSidebarProps> = ({ user }) => {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    e.preventDefault();
    console.log("[HabitHub Friend] 🔍 Searching for:", searchTerm);

    if (!searchTerm.includes('@')) {
       alert("이메일 형식을 정확히 입력해주세요.");
       console.log("[HabitHub Friend] ❌ Search invalid: No @ symbol");
       return;
    }
    ...
    try {
       ...
       console.log(`[HabitHub Friend] 🕵️‍♂️ Found ${allUsers.length} total users in DB`);
       ...
       console.log(`[HabitHub Friend] ✅ Matches found:`, found);
       
       setSearchResults(found);
    } catch (err) {
       console.error("[HabitHub Friend] ❌ Search error:", err);
    } finally {
  ...
  const sendRequest = async (receiverEmail: string) => {
    console.log(`[HabitHub Friend] 👉 'Add Friend' button clicked for: ${receiverEmail}`);
    ...
    // API Call
    try {
      console.log(`[HabitHub Friend] 🚀 Calling requestFriend API...`);
      const result = await requestFriend(user.email, receiverEmail);
      console.log(`[HabitHub Friend] 📥 requestFriend Result:`, result);
      ...
      setTimeout(() => {
          console.log(`[HabitHub Friend] 🔄 Reloading friends list...`);
          loadFriends();
      }, 1000);
      alert('요청을 보냈습니다.');
    } catch (e) {
      console.error(`[HabitHub Friend] ❌ Request failed:`, e);
  ...
  const handleDelete = async (friendEmail: string) => {
    console.log("[HabitHub Friend] 🗑️ Delete requested for:", friendEmail);
    if (confirm(`${friendEmail}님을 친구 목록에서 삭제하시겠습니까?`)) {
      ...
      console.log("[HabitHub Friend] 🚫 Delete cancelled by user");
    }
  };
          ...
          {activeTab === 'add' ? (
             <div className="space-y-4">
                ...
                {searchLoading ? (
                   ...
                   <div className="space-y-2">
                      ...
                      {searchTerm && searchResults.length === 0 && (
                         <div className="text-center text-xs text-github-muted py-2">검색 결과가 없습니다.</div>
                      )}
                   </div>
                )}
                ...
                {sentRequests.length > 0 && (
                   <div>
                      <h3 className="text-xs font-bold text-github-muted uppercase mb-2">보낸 요청</h3>
                      {sentRequests.map(f => (
             ...
             <div className="space-y-6">
                ...
                <div>
                   <h3 className="text-xs font-bold text-github-muted uppercase mb-3">내 친구 ({friendsWithStats.length})</h3>
                   {friendsWithStats.length === 0 ? (
                      <div className="text-center py-8 text-github-muted text-sm border border-dashed border-github-border rounded-lg">
                         아직 친구가 없습니다.<br/>친구를 추가해 서로 동기부여해보세요!
                      </div>
                   ) : (
                      <div className="space-y-4">
